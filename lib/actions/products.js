...
export const searchProducts = async (search = "") => {
  ...
  if (search.length > 3) {
    return await ProductModel.find({ $text: { $search: search } });
  } else {
    return await ProductModel.find({ name: { $regex: search, $options: "i" } });
  }
};
...
export const getProductsClient = async (
  ...
  try {
    ...
    const { products, totalPages } = await ProductModel.byCategoryWithFilters(
      categoryId,
      filter,
      page,
      limit
...
export async function createProduct(formData, images = []) {
  try {
    ...
    await connectDB();
    const { name, price, description, units, category, additionalCategories } =
      formData;
...
export async function changeUnits(id, units) {
  try {
    if (!(await verifySession())) throw new Error("No eres administrador");
    await connectDB();
    ...
    revalidatePath(`/`, "layout");
    return { message: "unidades actualizadas", success: true };
  } catch (error) {
