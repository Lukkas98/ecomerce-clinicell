...
export const getProducts = async (
  ...
  try {
    ...
    if (category && !search) {
      ...
      totalPages = await getTotalPages(null, categoryObj._id);
      ...
      products = await ProductModel.find()
        ...
        .exec();
      totalPages = await getTotalPages(search, null, filter);
    }
...
export const getTotalPages = async (search, categoryId, filter) => {
  const limit = 9;
  const query = {};
  if (search) query.name = { $regex: search, $options: "i" };
  if (categoryId) query.category = categoryId;
  if (filter) query.stock = filter === "stock" ? true : false;
