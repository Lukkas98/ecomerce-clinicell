...
import { cloudinary } from "@/app/admin/panel/[form]/components/actionCloudinary";
...
export const deleteImageByUrl = async (url, nameProduct) => {
  try {
    ...
    console.log(`Imagen ${publicId} eliminada exitosamente`);
    revalidatePath(`${Url}/admin/panel`);
...
const getPublicIdFromUrl = (url) => {
  try {
    ...
    // https://res.cloudinary.com/demo/image/upload/v1621877367/products/image.jpg
    ...
    // Extraer el public ID sin la extensión del archivo
    return matches[1].split("/").slice(-1)[0].split(".")[0];
  } catch (error) {
    throw new Error(`Error extrayendo public ID: ${error.message}`);
  }
};
