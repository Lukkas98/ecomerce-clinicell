...
export const getProductsClient = async (
  ...
  try {
    ...
    const totalPages = await getTotalPages(null, categoryId, null, limit);

    const products = await ProductModel.find()
      .byCategoryWithFilters(categoryId, filter, page, limit)
      .exec();
...
export const getProductsAdmin = async (search, filter = "az", page = 1) => {
  const limit = 8;
  ...
  try {
    await connectDB();
    const products = await ProductModel.find()
      .byFilters(search, filter, page)
      .exec();
    const totalPages = await getTotalPages(search, null, filter, limit, true);
...
export const getTotalPages = async (
  ...
  if (filter && isAdmin && (filter === "stock" || filter === "-stock")) {
...
export async function createProduct(formData, urls) {
  try {
    if (!verifySession()) throw Error("No eres administrador");
